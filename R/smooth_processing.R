...
parse_smoothers <- function(formula, data, knots = NULL, newdata = NULL, basis_prev = NULL) {
  ...
  if (length(smooth_i) > 0) {
    ...
    f_dims <- unlist(lapply(Xs, ncol))
    # Xs <- do.call(cbind, Xs) # combine 'em all into one design matrix
    smooth_cut <- c(0, cumsum(sm_dims)[-length(sm_dims)], sum(sm_dims)-1) + 1
    null_cut <- c(0, cumsum(f_dims)[-length(f_dims)], sum(f_dims)-1) + 1 
  } else {
    has_smooths <- FALSE
    sm_dims <- 0L
    f_dims <- 0L
  ...
  list(Xs = Xs, Zs = Zs, has_smooths = has_smooths, labels = labels,
       ...
       sm_dims = sm_dims, smooth_cut = smooth_cut, null_cut = null_cut)
}
